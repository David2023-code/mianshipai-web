...
一句话：Fiber 让 React 的“协调（Diff/计算更新）”变成一个可被调度的工作循环，把大任务切碎，在主线程上可暂停、可恢复、可插队地执行，从而减少卡顿。

1.  **两阶段（Render / Commit）**:
    - **Render（协调/计算）**: 遍历组件树、执行组件、对比新旧结构、标记副作用；这一步可以被中断、也可能被重做。
    - **Commit（提交/落地）**: 把副作用一次性应用到宿主环境（DOM/原生）；这一步不可中断，避免 UI 处于“改一半”的不一致状态。
2.  **工作单元（Unit of Work）**:
    - 把一次更新拆成很多小步骤，通常可理解为“处理一个 Fiber 节点就是一个工作单元”。
    - 每做完一个单元就有机会让出主线程，下一次再从上次进度继续。
3.  **可中断 + 时间切片（Time Slicing）**:
    - Fiber 把递归遍历改成可控的循环遍历，允许在“本帧时间不够”时暂停，先把控制权交还给浏览器处理输入/动画/绘制，再在后续时机续跑。
4.  **优先级调度（Scheduler + Lane）**:
    - 更新会被赋予不同优先级（如输入/点击 > 动画 > 普通更新 > 低优先级过渡）。
    - 高优先级可以打断低优先级，先保证交互响应，再补齐耗时渲染。
5.  **Fiber 节点（数据结构与状态载体）**:
    - Fiber 节点可理解为组件的“运行时节点/栈帧”，除了描述 UI，还保存更新过程需要的信息（如 state、更新队列、优先级、标记等）。
    - 通过 `child`（子）、`sibling`（兄）、`return`（父）形成链表式连接，便于中断与恢复遍历。
6.  **双缓存树（current / workInProgress）与提交**:
    - React 通常维护 current 树（屏幕上那棵）与 workInProgress 树（本次正在算的那棵）。
    - Render 产出 workInProgress 并把副作用打到节点上（如 Placement/Update/Deletion），Commit 阶段统一执行，然后切换整棵树为新的 current。

...
- Fiber 不只是“把任务切碎”，它同时是一种运行时数据结构：每个 Fiber 节点会保存组件的状态、更新队列、以及本次更新的标记与优先级信息。
- Fiber 让更新流程更清晰地分成 **Render（可中断，做 Diff/计算）** 与 **Commit（不可中断，做 DOM 落地）** 两阶段；Render 阶段算出 workInProgress 树与副作用标记，Commit 阶段一次性提交到页面。

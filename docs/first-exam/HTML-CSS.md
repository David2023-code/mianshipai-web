...
CSS 性能优化常从 3 个层面回答：选择器匹配、渲染成本、资源加载。

1. 选择器与样式计算

- 尽量用 class 选择器，减少层级过深的后代选择器
- 避免通用选择器 `*`、过度复杂的组合选择器
- 控制 DOM 规模与嵌套深度，DOM 越大样式计算越重

2. 减少重排/重绘成本

- 少改动会影响布局的属性（如 `width/height/top/left/margin`）
- 动画优先用 `transform` 和 `opacity`，避免用 `top/left` 做位移
- 大面积更新时使用 `contain` 限制影响范围（如 `contain: layout paint;`）

3. 降低绘制复杂度

- 谨慎使用 `filter/blur/backdrop-filter`、大面积 `box-shadow`、复杂渐变等高开销效果
- 长列表/复杂页面尽量分块渲染，避免一次性渲染过多内容

4. 资源与工程化

- 避免 `@import`（会阻塞并串行加载），使用 `<link rel="stylesheet">`
- 抽取首屏关键 CSS（critical CSS），非关键样式按路由/组件拆分按需加载
- 生产环境压缩与去冗余（minify、移除未使用 CSS），配合缓存与 CDN 提升加载性能

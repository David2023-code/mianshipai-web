...
2.  **ACK**: 服务端收到 FIN，发送 ACK=1，进入 CLOSE_WAIT。客户端收到后进入 FIN_WAIT_2。
    - _此时连接处于半关闭状态，服务端仍可发送数据。_
3.  **FIN**: 服务端数据发完了，发送 FIN=1，进入 LAST_ACK。
4.  **ACK**: 客户端收到 FIN，发送 ACK=1，进入 TIME_WAIT (等待 2MSL)，最后关闭。服务端收到 ACK 后立即关闭。

> **通俗理解**:
> A: 我没话说了，挂了吧 (FIN)
> B: 知道了 (ACK)。等一下，我还有几句要说... (数据传输) ...好了，我也没话说了，挂吧 (FIN)
> A: 好的，挂了 (ACK)
...
一句话背诵版：**解析 URL → 查缓存/DNS → 建连 (TCP/TLS) → 发 HTTP → 收响应/缓存 → 解析渲染 → 资源加载与交互就绪**。

1. URL 解析与安全策略

- 解析协议/域名/端口/路径/查询参数。
- 可能触发 HSTS：强制从 HTTP 升级到 HTTPS。

2. 缓存与拦截（能直接“就地返回”的优先）

- Service Worker：如果注册了 SW，可能先被它拦截并返回缓存/离线数据。
- 浏览器缓存：强缓存命中直接用；未命中再走协商缓存（`If-None-Match`/`If-Modified-Since`）。

3. DNS 解析

- 先查浏览器 DNS 缓存、系统缓存、路由器/运营商 DNS。
- 没有命中则递归解析，拿到目标 IP（可能是 CDN 边缘节点）。

4. 建立连接

- 复用已有连接：HTTP/1.1 keep-alive、HTTP/2 连接复用、HTTP/3(QUIC) 复用。
- 无可复用连接则新建：
  - TCP 三次握手（HTTPS 前置）。
  - HTTPS 还会进行 TLS 握手（协商版本/密钥/证书校验，可能有会话复用/0-RTT）。

5. 发送 HTTP 请求

- 组装请求行/请求头/cookie 等，可能先走代理或经由负载均衡。
- HTTP/2 会做多路复用；HTTP/1.1 同域并发连接数有限。

6. 服务端处理与返回响应

- 可能返回重定向（301/302/307/308），浏览器按 Location 再发起新请求。
- 返回响应头（缓存策略、压缩、CORS、Set-Cookie 等）与响应体（HTML/JSON/静态资源）。

7. 浏览器解析与渲染（以 HTML 页面为例）

- 解码与解析 HTML，构建 DOM。
- 解析 CSS，构建 CSSOM；DOM + CSSOM 合成 Render Tree。
- Layout（回流）计算布局，Paint（重绘）绘制，Composite 合成图层。
- 遇到外链资源（CSS/JS/图片/字体）继续发起请求；JS 可能阻塞解析（取决于 `defer/async`）。

8. 收尾与性能关键指标

- 资源加载完成、事件触发（DOMContentLoaded / load）。
- 可交互性相关：主线程是否被长任务占用，是否存在大量同步脚本、巨型 DOM、渲染抖动等。
